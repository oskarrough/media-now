...
const buildReleaseUrl = (id: string): string => `https://www.discogs.com/release/${id}`

/** Build Discogs master URL from ID */
const buildMasterUrl = (id: string): string => `https://www.discogs.com/master/${id}`
...
const fetchDiscogs = async (endpoint: string): Promise<Response> => {
  ...
  if (!response.ok) {
    throw new ProviderError('discogs', `HTTP ${response.status}: ${response.statusText}`)
  }
