...
import type { YouTubeResult, SearchResult } from '../types'
...
export const get = async (id: string): Promise<YouTubeResult> => {
  ...
  if (!response.ok) {
    throw new ProviderError('youtube', `HTTP ${response.status}: ${response.statusText}`)
  }
...
export const search = async (query: string): Promise<SearchResult[]> => {
  ...
  const contents =
    payload.contents?.twoColumnSearchResultsRenderer?.primaryContents?.sectionListRenderer?.contents ?? []

  const results: SearchResult[] = contents
    .flatMap((section) => section.itemSectionRenderer?.contents ?? [])
    .filter((item) => item.videoRenderer !== undefined)
    .map((item) => {
